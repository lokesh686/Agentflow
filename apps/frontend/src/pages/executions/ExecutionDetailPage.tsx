...
export default function ExecutionDetailPage() {
  ...
  const { current, isLoading, fetchOne, subscribe, cancel, approve } = useExecutionStore()
  ...
  return (
    <div className="p-8 max-w-4xl mx-auto">
      ...
      {current.status === 'PAUSED' && current.humanApproval?.required && !current.humanApproval.decision && (
        <div className="card border-orange-500/30 bg-orange-500/5 p-5 mb-4">
          <div className="flex items-center gap-2 mb-2">
            <span className="text-orange-400 text-lg">👤</span>
            <span className="font-semibold text-orange-400">Human approval required</span>
          </div>
          <p className="text-sm text-[#e8eaf0] mb-1">
            Agent: <strong>{current.humanApproval.agentName}</strong>
          </p>
          <p className="text-sm text-[#8891a8] mb-4">{current.humanApproval.context}</p>
          <div className="flex gap-3">
            <button onClick={() => approve(current._id, 'approved')}
              className="btn-primary text-sm">✓ Approve</button>
            <button onClick={() => approve(current._id, 'rejected')}
              className="btn-ghost text-sm text-red-400 hover:bg-red-500/10">✗ Reject</button>
          </div>
        </div>
      )}
